...
contract PerHookDataTest is CustomValidationTestBase {
    ...
    function test_failAccessControl_badSigData_runtime() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                uint8(MockAccessControlHookPlugin.ValidationId.PRE_VALIDATION_HOOK),
                abi.encodeWithSignature("Error(string)", "Proof doesn't match target")
            )
        );
        account1.executeWithAuthorization(
        ...
        returns (FunctionReference, bool, bool, bytes4[] memory, bytes memory, bytes memory, bytes memory)
    {
        FunctionReference accessControlHook = FunctionReferenceLib.pack(
            address(_accessControlHookPlugin), uint8(MockAccessControlHookPlugin.ValidationId.PRE_VALIDATION_HOOK)
        );
