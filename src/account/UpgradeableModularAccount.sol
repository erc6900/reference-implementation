        ...
        if (outerSelector == IStandardExecutor.execute.selector) {
            ...
            for (uint256 i = 0; i < calls.length; ++i) {
                if (calls[i].target == address(this)) {
                    bytes4 nesetedSelector = bytes4(calls[i].data);

                    if (
                        nesetedSelector == IStandardExecutor.execute.selector
                            || nesetedSelector == IStandardExecutor.executeBatch.selector
                    ) {
                    ...
                    _checkIfValidationAppliesSelector(nesetedSelector, validationFunction, isDefault);
                }
            }
        }
    }
