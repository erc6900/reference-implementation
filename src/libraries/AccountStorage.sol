...
struct SelectorData {
    ...
    address plugin;
    // Cached flags indicating whether or not this function has pre-execution hooks.
    // Flags for pre-validation hooks stored in the same storage word as the
    // validation function itself, to use a warm storage slot when loading.
